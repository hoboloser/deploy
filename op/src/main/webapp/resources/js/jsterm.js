...
(function() {
   ...
   var Terminal = {
      ...
      _addDirs: function(curDir, parentDir) {
		  var data = curDir.data;
		  if(curDir.data == undefined) {
			  data = curDir;
		  }
		 data.bind(this);
         /*data.contents.forEach(function(entry, i, dir) {
            if (entry.type == 'dir')
   ...
   term.init(CONFIG, 'http://172.16.30.35:8080/jdeploy/cmd/login', COMMANDS, function() {
      term.enqueue('login')
          .begin();
   });
